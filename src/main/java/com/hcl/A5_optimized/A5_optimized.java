...
public class A5_optimized {

	static int totalAgeM = 0;
	static int totalAgeF = 0;
	...
	public static void main(String[] args) {
		...
		empMap.put(new Employee(11, 47, 2009, 55000, "Mary", "Female", "Maintenance"), 47);
		
		// Prompt 1
		{

			Map<String, Long> mAndFGender = empMap.keySet().stream()
					.collect(Collectors.groupingBy(Employee::getGender, Collectors.counting()));

			System.out.println("Employee count: " + mAndFGender);
		...
		// Prompt 2
		{
			Map<String, Double> mAndFAge = empMap.keySet().stream()
					.collect(Collectors.groupingBy(Employee::getGender, Collectors.averagingInt(Employee::getAge)));

			System.out.println("\nAverage Age: " + mAndFAge);
		...
		// Prompt 3
		{
			Employee highestPaid = empMap.keySet().stream()
		...
		// Prompt 4
		{
			...
			empMap.keySet().stream().filter(e -> e.yearOfJoining > 2015)
					.forEach((e) -> System.out.print(e.name + ", "));
		}
		// Prompt 5
		{
			Employee oldest = empMap.keySet().stream().collect(Collectors.maxBy(Comparator.comparing(Employee::getAge)))
					.orElse(null);
			/*
			 * TreeMap<Employee, Integer> ageMap = new TreeMap<Employee, Integer>((e1, e2) -> { 
			 * if (e1.age == e2.age) { return 0; } else if (e1.age > e2.age) { return
			 * 1; } else return -1; }); 
		...
		// Prompt 6
		{
			Map<String, Long> perDept = empMap.keySet().stream()
					.collect(Collectors.groupingBy(Employee::getDept, Collectors.counting()));

			System.out.println("\nEmployees per department: " + perDept);
		...
		// Prompt 7
		{
			Map<String, Long> maintenance_Gender_Count = empMap.keySet().stream().filter(e -> e.dept == "Maintenance")
					.collect(Collectors.groupingBy(Employee::getGender, Collectors.counting()));

			System.out.println("Maintenance employees: " + maintenance_Gender_Count);
			 ...
			 */
			// System.out.println(String.format("\nMale and Female maintenance employees: %d male and %d female", maintenance_Male_Count, maintenance_Female_Count));
		}
		// Prompt 8
		{
			double avg = empMap.keySet().stream().collect(Collectors.averagingInt(Employee::getSalary));
			/*
			 * Iterator<Employee> itr = empMap.keySet().iterator(); 
			 * int totalSalary = 0;
			 * while (itr.hasNext()) { totalSalary += itr.next().salary; } 
		...
		{
			System.out.println("\n");
			Map<String, Long> byDept = empMap.keySet().stream()
					.collect(Collectors.groupingBy(Employee::getDept,Collectors.counting()));

			/*
			 * System.out.print("Employees in R & D: "); empMap.keySet().stream().filter((e) -> e.dept == "R & D").forEach((e) -> System.out.print(e.name + ", "));
			 * System.out.print("\nEmployees in Maintenance: ");
			 * empMap.keySet().stream().filter((e) -> e.dept == "Maintenance") .forEach((e)-> System.out.print(e.name + ", "));
			 * System.out.print("\nEmployees in HR: "); empMap.keySet().stream().filter((e)-> e.dept == "HR").forEach((e) -> System.out.print(e.name + ", "));
			 * System.out.print("\nEmployees in IT: "); empMap.keySet().stream().filter((e) -> e.dept == "IT").forEach((e) -> System.out.print(e.name + ", "));
			 */
		}
